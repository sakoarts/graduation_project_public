...
def my_config():
    ...
    genes_to_select = None #np.load('C:/TEMP/last_genes_intersection_10000.npy')
    ...
    predict_label = label_options[2]

    # The label on which the tt_split will be based
    split_label = label_options[2]
...
def model(classifier_name, classifier_parameters):
    ...
    if classifier_name is 'LinearSVC':
        model = OneVsRestClassifier(model, n_jobs=20)
